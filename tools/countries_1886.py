...
def write(mod="MOD", docs="MOD/documentation"):
    ...
    # Every tag that could exist at runtime, not just those owning a state.
    # Vanilla tags with no 1886 territory are still real -- GER, SOV, CHI and
    # the D01-D15 dynamic pool used for civil wars and released nations -- and
    # since this file replaces vanilla's they would otherwise have no colour
    # at all the moment one is created.
    # Only tags whose existence is grounded in something real: the ones this
    # mod defines, the ones observed owning a state in the recovered data, and
    # the D01-D15 dynamic pool, which the previous mod's own
    # zz_dynamic_countries.txt shows the game provides.
    #
    # NOT the hand-written VANILLA list. That list was already proven wrong once
    # -- it was missing 26 tags actually in use, including PRU -- so it cannot be
    # trusted in the other direction either. Naming a country that does not exist
    # in colors.txt is a risk taken on nothing but memory, and a vanilla tag left
    # without a colour is the milder failure of the two.
    DYNAMIC = {f"D{i:02d}" for i in range(1, 16)}
    for tag in sorted(_owner_tags() | DYNAMIC | {r[0] for r in ROWS}):
        if tag in have:
            continue
    ...
    with open(f"{mod}/common/countries/colors.txt", "w", encoding="utf-8") as fh:
        ...
        fh.write("# ones it uses included, plus the D01-D15 dynamic pool. Tags are only\n")
        fh.write("# listed where their existence is grounded in data, never in memory.\n\n")
