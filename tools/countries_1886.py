...
def write(mod="MOD", docs="MOD/documentation"):
    ...
    for tag in sorted(_owner_tags() | VANILLA | {r[0] for r in ROWS}):
        if tag in have:
            continue
    ...
    with open(f"{mod}/common/countries/colors.txt", "w", encoding="utf-8") as fh:
        ...
        fh.write("# ones included, and tags with no 1886 territory too: GER, SOV, CHI\n")
        fh.write("# and the D01-D15 dynamic pool are still real at runtime.\n\n")
